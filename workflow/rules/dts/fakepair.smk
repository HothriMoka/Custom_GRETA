# Check if fakepair config exists
if 'fakepitupair' in config['dts'] or 'fakepitunpair' in config['dts']:
...
# Only define this rule if pitupair is in config and fakepitupair is in config
if 'pitupair' in config['dts'] and 'fakepitupair' in config['dts']:
    localrules: annotate_fakepitupair
    rule annotate_fakepitupair:
        threads: 1
        singularity: 'workflow/envs/gretabench.sif'
        input:
            mdata='dts/pitupair/annotated.h5mu', # Direct path instead of rule reference
